#!/usr/bin/env python3
# from: https://github.com/mattvenn/librelane_summary/
...
if __name__ == '__main__':
    ...
    parser.add_argument('--copy', help='copy final Verilog and SDF', action='store_const', const=True)   # NOTE: edit
    ...
    # NOTE: edit
    if args.copy:
        path = check_path(os.path.join(run_path, 'final/pnl/*.v'))
        os.system(f"cp %s ../final/" % (path))
        path = check_path(os.path.join(run_path, 'final/sdf/nom_tt_025C_1v80/*.sdf'))
        os.system(f"cp %s ../final/" % (path))
